...
| The alpina.travel availability API | Called through a contained read-only adapter — a technical proof that an interface WordLift runs earns `agent-ready` like any other, with the provenance recorded as `via: "sidecar"`; enabling sites this way is future WordLift work, not part of the audit |
...
- `inspect-business-model` — the business as the audit modelled it from the pages it read: what it offers, where, who, each entity marked declared, inferred or human-confirmed, with the actions it answers for.
- `explain-entity` — one entity in full, by id or by name: provenance, pages, offers, links, the actions it answers for and the evidence behind them.
...
- `check-alpina-availability` — a contained read-only adapter for one allowlisted endpoint, kept as a technical proof of how an interface WordLift runs earns `agent-ready` (`via: "sidecar"`) in an immutable child revision. Turning that pattern into a product is future WordLift work, outside this audit.
