...
## Context-engine consolidation — 2026-08-30

- Product direction tightened from a travel demo to a site-agnostic context compiler. Alpina.travel remains one pinned fixture and one controlled read-only sidecar; no classification, entity extraction, action compilation, scoring, or report behavior depends on it.
- Andrea redirected the implementation toward end-to-end depth: “we need to leverage the full details we get from WordLift audit API without interrupting the user flow,” analyze three or four meaningful pages, and make the connection between entities and actions visually explicit.
- Andrea then clarified where site classification belongs: it is the input that selects the expected action model, not merely a report label.
- The live collection model now selects up to four complementary pages, extracts normalized JSON-LD entities and offers with page provenance, and preserves full bounded WordLift audit sections and quick wins behind progressive disclosure.
- The report contract now distinguishes `DomainEntity`, lexical context, `ActionInterface`, and `EntityActionBinding`. A fallback `WebSite` provider keeps the capability map useful for sites with weak structured data without fabricating specific products or services.
- The UI now presents Domain graph → Lexical graph → Action layer, lets a reviewer select an entity, and highlights bound action nodes with entity chips and implementation evidence.
- Generic compiler assertions cover commerce/products, publishers/articles, finance/services, SaaS/applications, travel/lodging, and the conservative other/unstructured path.
- Narrative artifacts were rewritten around WordLift's context engine and the generic URL-first workflow. The stable Alpina route is positioned only as the final runtime proof.
