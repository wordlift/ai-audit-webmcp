...
export function ServiceMapProvenance({ report }: { report: ReportRecord }) {
  ...
  if (report.refinement) {
    ...
    const entityName = (id: string) => report.contextGraph?.entities.find((entity) => entity.id === id)?.name ?? id;
    const label = (actionId: string) => report.capabilities?.find((capability) => capability.actionId === actionId)?.label ?? actionId;
    return (
      <section className="map-provenance map-provenance-refined" aria-label="Terms of Action provenance">
        <div className="map-provenance-head">
          <p className="map-provenance-title"><UserRoundCheck aria-hidden="true" /> Human-refined Terms of Action</p>
        ...
        {(() => {
          ...
          return <div className="map-stats" aria-label="What changed">{stats.map((entry) => <span key={entry} className="map-stat">{entry}</span>)}</div>;
        })()}
        <details className="map-diff">
          <summary>Full decision log</summary>
          <ul className="map-changes">
            {(assertions.primaryEntityIds ?? []).length > 0 && <li><span className="provenance-badge">Human-provided</span> Primary entities: {(assertions.primaryEntityIds ?? []).map(entityName).join(", ")}</li>}
            {(assertions.demotedEntityIds ?? []).length > 0 && <li><span className="provenance-badge">Human-provided</span> Demoted: {(assertions.demotedEntityIds ?? []).map(entityName).join(", ")}</li>}
            {(assertions.terminology ?? []).map((entry) => <li key={entry.term}><span className="provenance-badge">Human-provided</span> "{entry.term}" means {entry.meaning}</li>)}
            {(assertions.terminologyDecisions ?? []).map((decision) => <li key={decision.term}><span className="provenance-badge">Human-provided</span> "{decision.term}" {decision.decision === "reject" ? "rejected as machine vocabulary" : decision.decision === "confirm" ? "confirmed" : `now means ${decision.meaning}`}</li>)}
            {(assertions.actionDecisions ?? []).map((decision) => (
              <li key={decision.actionId}><span className="provenance-badge">Human-provided</span> {label(decision.actionId)}: {decision.decision === "confirm" ? "confirmed" : "rejected"}{decision.boundary ? ` · ${BOUNDARY_LABELS[decision.boundary]}` : ""}{decision.rationale ? <em> — {decision.rationale}</em> : null}</li>
            ))}
          </ul>
        </details>
        {refinement.conflicts.length > 0 && <p className="map-conflicts">Not applied: {refinement.conflicts.join(" · ")}</p>}
      </section>
    );
  }

  return (
    <section className="map-provenance map-provenance-draft" aria-label="Terms of Action provenance">
      <div className="map-provenance-head">
        <p className="map-provenance-title"><Bot aria-hidden="true" /> Machine-generated Terms of Action</p>
        <p className="map-provenance-copy">
          Open this report in ChatGPT's built-in browser: ChatGPT will inspect the machine draft, interview you
          about the business — its role, its vocabulary, who owns each action — and compile human-refined Terms of Action.
        </p>
      </div>
