...
export class FirestoreClaimStore implements ClaimStore {
  ...
  static fromProject(projectId?: string, prefix = "") {
    return new FirestoreClaimStore(new Firestore({ ignoreUndefinedProperties: true, ...(projectId ? { projectId } : {}) }), undefined, prefix);
  }

  async put(input: ReportClaim): Promise<ReportClaim> {
    const claim = reportClaimSchema.parse(input);
    await this.firestore.collection(`${this.prefix}reportClaims`).doc(claim.reportId).set(claim);
    return claim;
  }

  async get(reportId: string): Promise<ReportClaim | null> {
    const snapshot = await this.firestore.collection(`${this.prefix}reportClaims`).doc(reportId).get();
